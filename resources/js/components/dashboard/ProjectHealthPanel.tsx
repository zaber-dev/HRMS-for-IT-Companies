...
const STATUS_CONFIG: Record<ProjectStatus, { label: string; colorClass: string }> = {
    planning: { label: 'Planning', colorClass: 'text-blue-600 dark:text-blue-400' },
    in_progress: { label: 'In Progress', colorClass: 'text-yellow-600 dark:text-yellow-400' },
    on_hold: { label: 'On Hold', colorClass: 'text-orange-600 dark:text-orange-400' },
    completed: { label: 'Completed', colorClass: 'text-green-600 dark:text-green-400' },
    cancelled: { label: 'Cancelled', colorClass: 'text-gray-500 dark:text-gray-400' },
};
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
                {STATUS_ORDER.map((status) => {
                    ...
                    return (
                        ...
                        >
                            <div className={`text-2xl font-bold ${config.colorClass}`}>{count}</div>
                ...
                <div className="rounded-md border border-red-200 bg-red-50 p-3 text-center dark:border-red-900/30 dark:bg-red-900/10">
                    ...
                    <div className="mt-1 text-xs text-muted-foreground">Overdue</div>
                </div>
            </div>
        </div>
    );
