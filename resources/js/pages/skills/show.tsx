...
import { index, show, edit, destroy, toggle } from '@/actions/App/Http/Controllers/Skills/SkillController';
import Heading from '@/components/heading';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { type Skill, type SkillAssignment, type SkillCategory, type User } from '@/types';

type Props = {
    skill: Skill & {
        skill_category: SkillCategory;
        assignments: (SkillAssignment & { user: Pick<User, 'id' | 'name' | 'email'> })[];
    };
    ...
    return (
        <>
            ...
            <div className="space-y-8 max-w-2xl">
                <Heading title={skill.name} description="Skill details and assigned employees" />

                {/* Skill Details */}
                <div className="rounded-lg border border-border p-6 space-y-4">
                    <h2 className="text-base font-semibold">Skill Information</h2>

                    <dl className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
                        <div>
                            <dt className="text-muted-foreground">Name</dt>
                            <dd className="font-medium mt-1">{skill.name}</dd>
                        </div>
                        <div>
                        ...
                        <div className="col-span-2">
                            ...
                            <dd className="mt-1">
                                {skill.description ?? <span className="italic text-muted-foreground">No description provided</span>}
                            </dd>
                        </div>
                    </dl>
                </div>
                ...
                <div className="space-y-4">
                    ...
                    {skill.assignments.length > 0 ? (
                        ...
                        <p className="text-sm text-muted-foreground">No employees assigned to this skill yet.</p>
                    )}
                </div>
            </div>
        </>
